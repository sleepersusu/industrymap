...
1. **第一輪漏掉的不是資料，是查詢角度。** 銅箔基板、散熱片、連接器這幾類，第一輪
   ...
   而不是等人看出來。
2. **掛零有時是節點設計的問題，不是資料的問題。** `供電模組` 查不到供應商，是因為它
   在主機板上不是採購件，而是板廠用 MOSFET／電感／電容／PWM 焊出來的子組件。
   真正被採購的是**整合驅動器與功率 MOSFET 的功率級**，因此本輪新增 `DrMOS`（id 262）
   掛在供電模組下（`COMMON`——高階板標配，入門板仍用分離式），供應商 Infineon、MPS、Renesas。
   **`供電模組` 維持 0 供應商是正確結果**，不要為了把數字補滿而硬塞。
   日後再遇到掛零節點，先問「這是採購件還是組裝出來的」，再決定要補公司還是補子節點。
