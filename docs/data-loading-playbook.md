...
### G1：列表只解了一半——「列出所有節點（含零件）」仍需查資料庫

**已解掉的一半**：`GET /api/products` 已上線（`add-item-listing-and-amendment`），
分頁列出**終端成品**，支援名稱模糊搜尋與 `includeDrafts`，已駁回一律不外露。
「使用者／前端點不進來，只能有人先告訴你 id 是 241」這個問題已經沒有了。

公司側也已補上（`add-company-listing`）：`GET /api/companies` 分頁列出公司，
名稱關鍵字同時比對名稱與別名，並可依國別、公開發行狀態、供應的品類節點過濾。
「台灣有哪些公司在做 PCB」這類跨產品彙總的問題，已不必再查資料庫。

**仍缺的一半**：**沒有列出零件節點的端點**。步驟 2 要撈既有的**節點**清單當 background 去重，
**仍然只能直接查資料庫（唯讀）**；公司清單則已可改走 API。
`GET /api/items` 的行為不變，維持以名稱或別名解析**單筆**。

影響：紀律 1 的節點去重、方案 A 的 pipeline 在生成前取既有節點，都還是靠 `psql`。

這是刻意的取捨，不是遺漏——把 `GET /api/items` 改成列表是 breaking change
（紀律 1 的第二道防線指令、灌資料腳本、既有測試全要改），而當時的痛點是進入點，
用不到列零件。日後真要列零件時走既定遷移路徑（見該 change 的 design D1 方案 C）：
先加 `GET /api/items/resolve?name=` 承接單筆解析 → 兩者並存一段時間 →
更新本手冊與腳本 → 最後才讓 `GET /api/items` 轉為列表。
