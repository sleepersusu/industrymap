...
- (company) 新增公司列表 `GET /api/companies`：分頁列出公司，名稱關鍵字會同時比對公司名稱與別名
  （以 TSMC 查得到台積電），並可依國別（不分大小寫）、是否公開發行、供應的零件與角色過濾，條件可併用。
  已駁回公司任何條件下都不外露，無資料時回空清單而非 404。此前只能從產品側往下展開，
  「台灣有哪些公司在做 PCB」這類問題資料都在卻沒有入口，反查公司必須先知道代號 — 2026-07-28 · user · (pending)
